...
Author: Martina Kutmon & Grace Marshall
...
rownames(annotate) = make.names(annotate$hgnc_symbol, unique=TRUE)
...
combined_stage_all_1$gene_id<-gene_id[,2]
rownames(combined_stage_all_1)<-make.names(combined_stage_all_1$gene_id, unique=TRUE)
...
trait_data$stage_1<-trait_data$stage
colnames(trait_data)<-c("samples","stage","stage_1")

# Form a data frame analogous to expression data that will hold the clinical traits.

#samples = rownames(combined.filtered);
#traitRows = match(samples, trait_data$samples);
#datTraits = as.data.franetrait_data[traitRows, -1];
...
```{r}
save(combined.filtered, trait_data,trait_data_new, file = "WGCNA-input.RData")
```
...
sft = pickSoftThreshold(combined.filtered, powerVector = powers, verbose = 5,networkType = "unsigned")
# Plot the results:
sizeGrWindow(9, 5)
par(mfrow = c(1,2));
cex1 = 0.9;
...
#NS1=networkScreening(y=trait_data$stage, datME=MEs,
datExpr=combined.filtered, oddPower=3, blockSize=2000,
minimumSampleSize=4,addMEy=TRUE, removeDiag=FALSE,
weightESy=1, getQValues=TRUE)
```
...
row.names=F,sep=",")
datMEy = data.frame(trait_data$stage, MEs)
eigengeneSignificance1 = cor(datMEy, trait_data$stage)
...
dimnames(out1)[[1]][-c(1:3)]=dimnames(combined.filtered)[[1]]
...
TOM = TOMsimilarityFromExpr(combined.filtered, power = 5);
...
modGenes = annotate_green$hgnc_symbol[match(modProbes, annotate_green$entrezgene_id)];
